...
CREATE TABLE public.shop_sync_state (
  ...
  CONSTRAINT shop_sync_state_pkey PRIMARY KEY (shop_id)
);
CREATE TABLE public.assistant_chats (
  ...
  CONSTRAINT assistant_chats_shop_id_fkey FOREIGN KEY (shop_id) REFERENCES public.shops(id),
  CONSTRAINT assistant_chats_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
